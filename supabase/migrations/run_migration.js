...
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://pjzujrwbfwcxdnjnuhws.supabase.co';
...
const migrationFilePath = path.join(__dirname, '20250314_create_users_table.sql');
...
async function migrateUsers() {
  try {
    ...
    for (const user of mockUsers) {
      try {
        ...
        if (insertError) {
          ...
          console.log(`Could not insert user ${user.email}, trying update...`);
    ...
    console.log(`Migration complete: ${successCount} users migrated successfully, ${errorCount} failed`);
    return { success: successCount > 0, message: `Migrated ${successCount} users successfully${errorCount > 0 ? `, ${errorCount} failed` : ''}` };
  } catch (error) {
