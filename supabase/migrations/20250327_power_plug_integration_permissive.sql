...
-- Allow any authenticated user to insert power sensors
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_sensors'
    AND policyname = 'Allow authenticated users to insert power_sensors'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to insert power_sensors" ON public.power_sensors FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
  END IF;
END
$$;

-- Allow any authenticated user to update power sensors
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_sensors'
    AND policyname = 'Allow authenticated users to update power_sensors'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to update power_sensors" ON public.power_sensors FOR UPDATE USING (auth.role() = ''authenticated'')';
  END IF;
END
$$;

-- Allow any authenticated user to delete power sensors
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_sensors'
    AND policyname = 'Allow authenticated users to delete power_sensors'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to delete power_sensors" ON public.power_sensors FOR DELETE USING (auth.role() = ''authenticated'')';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_consumption'
    AND policyname = 'Allow authenticated users to insert power_consumption'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to insert power_consumption" ON public.power_consumption FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_status'
    AND policyname = 'Allow authenticated users to insert power_status'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to insert power_status" ON public.power_status FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_status'
    AND policyname = 'Allow authenticated users to update power_status'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to update power_status" ON public.power_status FOR UPDATE USING (auth.role() = ''authenticated'')';
  END IF;
END
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies
    WHERE tablename = 'power_audit_log'
    AND policyname = 'Allow authenticated users to insert power_audit_log'
  ) THEN
    EXECUTE 'CREATE POLICY "Allow authenticated users to insert power_audit_log" ON public.power_audit_log FOR INSERT WITH CHECK (auth.role() = ''authenticated'')';
  END IF;
END
$$;
