...
const SensorDataGraphs: React.FC<SensorDataGraphsProps> = ({
	...
	const [sensorInfoMap, setSensorInfoMap] = useState<
	...
	const handleDataSelectionConfirm = async (selectedDataTypes: string[]) => {
		try {
			...
			const updatedProject = await downloadProjectReport(
				project,
				selectedDataTypes
			);
					...
					return (
						<div key={sensorImei} className="space-y-4">
							...
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								{Object.entries(valueConfigs).map(
									([key, config]) => {
										const currentValue =
											latestData.values[
												key as keyof typeof latestData.values
											];
