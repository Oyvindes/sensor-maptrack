...
const AssignedSensorsList: React.FC<AssignedSensorsListProps> = ({
  ...
  const assignedSensorIds = project.assignedSensorIds || [];
  
  const handleUnassignSensor = (sensorId: string) => {
    toast.success(`Sensor ${sensorId} unassigned from project`);
    ...
    <Card className={cn("w-full", className)}>
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          <CardTitle className="text-base">Assigned Sensors</CardTitle>
          <Button 
            variant="ghost" 
            size="sm" 
            className="h-8 px-2"
      ...
      <CardContent>
        {assignedSensorIds.length === 0 ? (
          <div className="text-center p-4 text-muted-foreground text-sm">
            No sensors assigned to this project
          </div>
        ) : (
          <div className="space-y-2">
            {assignedSensorIds.map((sensorId) => (
              <div 
                key={sensorId}
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm">{sensorId}</span>
                </div>
                <div className="flex items-center gap-1">
                  <Button 
                    ...
                    onClick={() => toast.info(`Viewing sensor ${sensorId}`)}
                  >
                  ...
                  <Button 
                    ...
                    onClick={() => handleUnassignSensor(sensorId)}
                  >
                    <X className="h-3 w-3" />
                  </Button>
                </div>
