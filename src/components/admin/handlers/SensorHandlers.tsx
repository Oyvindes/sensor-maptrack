...
import {
	saveSensor,
	deleteSensor
} from '@/services/sensor/supabaseSensorService';
...
export function useSensorHandlers(
	...
	const handleImportSensors = async (
		...
		const existingImeis = sensors.filter((s) => s.imei).map((s) => s.imei);

		const newSensors = importedSensors.filter(
			(s) => !existingImeis.includes(s.imei)
		);
		...
		if (savedSensors.length > 0) {
			...
			toast.info(
				`All ${duplicates} sensors already exist in the database`
			);
		}
	...
	const handleDeleteSensors = async (
		...
		const sensorsToDelete = sensors.filter(
			(sensor) =>
				sensor.imei &&
				sensorIdentifiers.some(
					(identifier) => identifier.imei === sensor.imei
				)
		);
		...
		const unauthorizedSensors = sensorsToDelete.filter(
			(sensor) => !canEditSensor(sensor)
		);
		if (unauthorizedSensors.length > 0) {
			toast.error(
				`You don't have permission to delete ${unauthorizedSensors.length} sensors`
			);
			return;
		}
		...
		for (const sensor of sensorsToDelete) {
			try {
				const result = await deleteSensor(sensor.imei);
				if (result.success) {
		...
		if (deletedSensors.length > 0) {
			setSensors((prevSensors) =>
				prevSensors.filter(
					(s) => !deletedSensors.some((ds) => ds.id === s.id)
				)
			);
		}
	...
	const canEditSensor = (
		...
		if (
			...
			sensor.companyId === currentUser.companyId
		)
			return true;
